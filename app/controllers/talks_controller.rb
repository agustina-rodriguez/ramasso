class TalksController < ApplicationController
  ...
  def index
    @talks = Talk.all
    @talks = @talks.order(:name).page(params[:page] || 1)
    @talks = @talks.where(['speakers.first_name LIKE :filter OR speakers.last_name LIKE :filter', :filter => ('%' + params[:filter] + '%')]) unless params[:filter].blank?
  end
  ...
  private
    ...
    def talk_params
      params.require(:talk).permit(:name, :schedule, :event_id)
    end
end
